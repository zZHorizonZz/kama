...
public class CollectionServiceImplTest {
    ...
    private static JsonObject baseCollection() {
        ...
        Collection col = Collection.newBuilder()
                ...
                .build();
        return ResourceUtil.encodeCollection(col);
    }
    ...
    void testCreateAndGet() throws Exception {
        ...
        Collection createdProto = ResourceUtil.decodeCollection(created);
        ...
        Collection fetchedProto = ResourceUtil.decodeCollection(fetched);
    ...
    void testListAndUpdateAndDelete() throws Exception {
        ...
        Collection p1 = ResourceUtil.decodeCollection(c1);
        Collection p2 = ResourceUtil.decodeCollection(c2);

        List<JsonObject> list = service.listCollections().await(5, TimeUnit.SECONDS);
        assertTrue(list.size() >= 2);

        // Update display name of first
        Collection updated = Collection.newBuilder(ResourceUtil.decodeCollection(c1))
                .setDisplayName("Updated Name")
                .build();
        JsonObject updatedJson = service.updateCollection(ResourceUtil.encodeCollection(updated)).await(5, TimeUnit.SECONDS);
        Collection updatedProto = ResourceUtil.decodeCollection(updatedJson);
