...
import dev.cloudeko.kama.record.v1.RecordServerProto;
...
public class RecordServiceImpl implements RecordService, Service {
    ...
    public void bind(GrpcServer server) {
        server.callHandler(dev.cloudeko.kama.server.handler.CreateRecordV1Handler.SERVICE_METHOD, new dev.cloudeko.kama.server.handler.CreateRecordV1Handler(this));
        server.callHandler(dev.cloudeko.kama.server.handler.GetRecordV1Handler.SERVICE_METHOD, new dev.cloudeko.kama.server.handler.GetRecordV1Handler(this));
        server.callHandler(dev.cloudeko.kama.server.handler.ListRecordsV1Handler.SERVICE_METHOD, new dev.cloudeko.kama.server.handler.ListRecordsV1Handler(this));
        server.callHandler(dev.cloudeko.kama.server.handler.UpdateRecordV1Handler.SERVICE_METHOD, new dev.cloudeko.kama.server.handler.UpdateRecordV1Handler(this));
        server.callHandler(dev.cloudeko.kama.server.handler.DeleteRecordV1Handler.SERVICE_METHOD, new dev.cloudeko.kama.server.handler.DeleteRecordV1Handler(this));
    }

    private static final ServiceName V1_SERVICE_NAME = ServiceName.create("cloudeko.kama.record.v1.RecordService");
    private static final Descriptors.ServiceDescriptor V1_SERVICE_DESCRIPTOR = RecordServerProto.getDescriptor().findServiceByName("RecordService");

    private final Vertx vertx;
    private final SqlClient client;

    public RecordServiceImpl(Vertx vertx, DatabaseOptions options) {
        this.vertx = vertx;
    ...
    private Future<Collection> fetchCollection(String name) {
        ...
        return SqlTemplate.forQuery(client, sql).execute(Map.of("name", name))
                .compose(rs -> {
                    ...
                    if (!it.hasNext()) {
                        return Future.failedFuture("Collection not found");
                    }
                    Row row = it.next();
                    return Future.succeededFuture(new JsonObject(row.getString("schema_json")));
                })
                .compose(v -> SqlTemplate.forQuery(client, sql).execute(Map.of("name", name)))
                .map(rowSet -> {
                    Row row = rowSet.iterator().next();
                    String schema = row.getString(row.getColumnIndex("schema_json") != -1 ? "schema_json" : "SCHEMA_JSON");
                    return ResourceUtil.decode(new JsonObject(schema));
                });
    }
    ...
    public Future<JsonObject> createRecord(String parent, JsonObject record) {
        ...
        return fetchCollection(collectionName).compose(collection -> {
            ...
            return SqlTemplate.forUpdate(client, sql.toString()).execute(params).map(v -> {
                ...
                JsonObject out = ResourceUtil.encode(toStore);
            ...
            return SqlTemplate.forQuery(client, sql).execute(Map.of("id", id))
                    .compose(rs -> {
                        ...
                        dev.cloudeko.kama.record.v1.Record rec = dev.cloudeko.kama.record.v1.Record.newBuilder()
                                ...
                                .setCollection(collectionName)
                                .build();
                        JsonObject out = ResourceUtil.encode(rec);
    ...
    public Future<List<JsonObject>> listRecords(String parent) {
        ...
        return fetchCollection(parent).compose(col -> {
            String tbl = tableNameFor(col);
                    ...
                    .map(rowSet -> {
                        ...
                        for (Row row : rowSet) {
                            ...
                            dev.cloudeko.kama.record.v1.Record rec = dev.cloudeko.kama.record.v1.Record.newBuilder()
                                    ...
                                    .build();
                            out.add(ResourceUtil.encode(rec));
                        }
                        return out;
                    });
        });
    ...
    public Future<Void> deleteRecord(String name) {
        ...
        String collectionName = name.substring(0, idx);
        String id = name.substring(idx + "/records/".length());
        return fetchCollection(collectionName).compose(col -> {
            String tbl = tableNameFor(col);
